...
def generarClaseNacional(clase):
    ...
    if lista_comunidades_presentes:
        ...
        cmd=f"ogr2ogr -f \"FlatGeobuf\" -nln {clase} -dialect sqlite -sql \"select distinct ST_MakeValid(geometry),* from {clase}\" {var_dict['path_generados_nacional_municipios']}00/{clase}.fgb {var_dict['path_generados_nacional_municipios']}00/{clase}_recorte.vrt >/dev/null 2>&1"
