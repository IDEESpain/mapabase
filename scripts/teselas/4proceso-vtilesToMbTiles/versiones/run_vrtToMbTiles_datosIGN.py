...
class ProcessIGO:
    def __init__(self):
        ...
        with open('config_vtiles_datosIGN.json') as data_file:
            self.config_vtiles = json.load(data_file)

        with open('config_datosIGN.json') as data_file:
    ...
    def compress_geojson(self):
        ...
        input_directory = self.config["input_vrt"]
        output_directory = self.config["gz_folder"]
    ...
    def tiling_pbf(self):
        ...
        # def teselacion(z):
            ...
            input_layers = self.config["gz_folder"]
    ...
    def move_temp_files(self):
        ...
        def mover_nivel(i,temp_path,temp_path_children,father_dest_path):
            ...
            shutil.copytree(cnig_folder_path, father_dest_path, dirs_exist_ok=True)


        ## TODO PARAMETRIZAR Y TRATAR DE HACERLO POR LAS X
        Parallel(n_jobs=6, require='sharedmem')(delayed(mover_nivel)(i,temp_path,temp_path_children,father_dest_path) for i in range(len(temp_path_children)))
    ...
    def combine_json(self):
        ...
        dic_output_json['center'] = """{0},{1},{2}""".format(self.compute_max_min_avg(center_tuple_lst, 'avg', 0), self.compute_max_min_avg(center_tuple_lst, 'avg', 1), self.compute_max_min_avg(center_tuple_lst, 'min', 2))
        dic_output_json['bounds'] = """{0},{1},{2},{3}""".format(self.compute_max_min_avg(bounds_tuple_lst, 'avg', 0), self.compute_max_min_avg(bounds_tuple_lst, 'avg', 1), self.compute_max_min_avg(bounds_tuple_lst, 'avg', 2), self.compute_max_min_avg(bounds_tuple_lst, 'avg', 3))
        ...
        dic_output_json['generator'] = self.check_different_elements(df_final['generator'].to_list())
        ...
        for zoom,element in lst_json:
            for vector_layer in element["vector_layers"]:
                if vector_layer["id"] not in capas_totales.keys():
                    ...
                    elemento={"id":vector_layer['id'],"minzoom":vector_layer['minzoom'],"maxzoom":vector_layer['maxzoom'],"description":vector_layer['description'],"atributos":vector_layer['fields']}
                ...
                capas_totales[vector_layer["id"]]["atributos"]={**lista_atributos_completa, **lista_atributos_zoom}
...
#detectar si es para actualizar o tirar completo
print(len(sys.argv))
...
if(len(sys.argv)>1 and sys.argv[1]=="update"):
    ...
    print(f"Actualizando teselas de territorios")

    lista_comunidades=config["lista_comunidades"]
    lista_municipios=[46176]
    ...
    if(len(sys.argv)>2 and sys.argv[2]=="comunidades"):
        is_comunidades=True
    ...
    def get_territorio(is_comunidades,var_dict,codigo):
        if is_comunidades:
            ...
            territorio = comunidades_file[comunidades_file["codigo"].str.slice(2,4)==str(codigo).zfill(2)]

        else:

            file=f"{var_dict['aux_municipios_file']}"
            municipios_file=gpd.read_file(file)
            territorio = municipios_file[municipios_file["codigo"].str.slice(-5)==str(codigo).zfill(5)]

            # ruta_carpeta=f"{var_dict['path_generados_municipios']}"
            # territorio_cod=codigo
            # file=f"{ruta_carpeta}{territorio_cod}/{territorio_cod}.fgb"
            # territorio=gpd.read_file(file)
    ...
    def get_tiles(left,bottom,right,top):
        ...
        neigbour_tiles_matrix=[(t.x,t.y,t.z) for l in neigbour_tiles for t in l]
        ...
        total_tiles_matrix_set_sorted_by_zoom = sorted(total_tiles_matrix_set, key=lambda tup: tup[2])

        total_tiles_dict = {i:{"x":t[0],"y":t[1],"z":t[2]} for i,t in enumerate(total_tiles_matrix_set_sorted_by_zoom)}
    ...
    def parse_json_v_tiles():
        ...
        for l in config_vtiles["zoom_levels"]:
            ...
            layers_no_coalesce=[layer["file"] for layer in l["layers_no_coalesce"]]
            layers_with_labels=[layer["file"] for layer in l["layers_with_labels"]]
            filters=l["filter"]
            #print(layers)
            layer_dict_by_zoom[l["level"]]={"layers":layers,"layers_no_coalesce":layers_no_coalesce,"layers_with_labels":layers_with_labels,"filter":filters}
    ...
    def generate_tiles_tippecanoe(tiles_df,layer_dict_by_zoom):
        ...
        origen= config["gz_folder"]


        #print(layer_dict_by_zoom)

        for t in tiles_df.itertuples():
            layers=["--named-layer="+layer[:-3]+":"+origen+layer for layer in layer_dict_by_zoom[t.z]["layers"]]
            layers_no_coalesce=["--named-layer="+layer[:-3]+":"+origen+layer for layer in layer_dict_by_zoom[t.z]["layers_no_coalesce"]]
            layers_with_labels=["--named-layer="+layer[:-3]+":"+origen+layer for layer in layer_dict_by_zoom[t.z]["layers_with_labels"]]
            filter_attr = json.dumps(t.z["filter"])

            layers=" ".join(layers)


            command_line = f"tippecanoe -P -o " + destination_mbtiles+"CNIG_" + str(t.z) +"_"+str(t.x)+"_"+str(t.y)+"_layers.mbtiles " + \
                            layers + \
                            " -z " + str(t.z) + " -Z " + str(t.z) + \
                            " -j '" + filter_attr + "'" + \
                            " --buffer=44" + \
            ...
            if len(layers_no_coalesce)>0:
                ...
                command_line = f"tippecanoe -P -o " + destination_mbtiles+"CNIG_" + str(t.z) +"_"+str(t.x)+"_"+str(t.y)+"_layers_no_coalesce.mbtiles " + \
                                layers_no_coalesce + \
                                " -z " + str(t.z) + " -Z " + str(t.z) + \
                                " -j '" + filter_attr + "'" + \
                                " --buffer=44" + \
            ...
            if len(layers_with_labels)>0:
                ...
                command_line = f"tippecanoe -P -o " + destination_mbtiles+"CNIG_" + str(t.z) +"_"+str(t.x)+"_"+str(t.y)+"_layers_with_labels.mbtiles " + \
                                layers_with_labels + \
                                " -z " + str(t.z) + " -Z " + str(t.z) + \
                                " -j '" + filter_attr + "'" + \
                                " --buffer=127" + \
    ...
    def grouped_mbtiles_by_zoom(mbtiles_path):
        mbtiles_files_dict={i:{"CNIG":f.split("_")[0],"zoom":f.split("_")[1],"x":f.split("_")[2],"y":f.split("_")[3],"layer_type":f.split("_")[4].split(".")[0],"file_type":"."+f.split("_")[4].split(".")[1]} for i,f in enumerate(os.listdir(mbtiles_path)) if f[-8:]==".mbtiles"}
        df_mbtiles_files=pd.DataFrame.from_dict(mbtiles_files_dict,orient="index").sort_values(["zoom","x","y"])
